...
from steps import one_pass_gan_steps as step
...
def tranform_avg1(data, mode, scale='scale', sim_lr=sim_lr, sim_hr=sim_hr):
    if mode=='hr':
...
for i in range(stack_size):
    train_stack[i*p.x_dim:(i+1)*p.x_dim,:,:,0] = tranform_avg1(np.load(p.loc_training_tiles.format(i)).astype(np.float32), mode='lr')
    label_stack[i*p.x_dim_out:(i+1)*p.x_dim_out,:,:,0] = tranform_avg1(np.load(p.loc_label_tiles.format(i)).astype(np.float32), mode='hr')
...
generator_model = tf.keras.Model(inputs = generator_input, outputs = p.model.first_generator(generator_input))

critic_input1 = layers.Input(shape=(p.x_dim_out, p.y_dim_out, 1))
critic_input2 = layers.Input(shape=(p.x_dim_out, p.y_dim_out, 1))
critic_model = tf.keras.Model(inputs = (critic_input1, critic_input2), outputs = p.model.first_critic(critic_input1, critic_input2))
...
test_slice = transform_pol(sim[42,...])
    ...
    for epoch in range(epochs):
        ...
        for train_batch, label_batch in zip(train_dataset, label_dataset):
            ...
            if batch_counter % p.k_crit != 0:
                crit_loss, crit_gradients = step.first_train_step_crit(train_batch, label_batch, 
                                                                        generator_model, critic_model, 
                                                                        p.first_crit_loss, p.first_critic_optimizer)

            elif batch_counter % p.k_crit == 0:
                (gen_loss, crit_loss, gen_gradients, crit_gradients) = step.first_train_step(train_batch, label_batch,
                                                                        generator_model, critic_model, 
            ...
            with train_summary_writer.as_default():
                tf.summary.scalar('critic loss', crit_loss, step=epoch)
                crit_tensor_counter=0
                for tensor in crit_gradients:
                    tf.summary.histogram('critic gradients tensor {}'.format(crit_tensor_counter), tensor, step=epoch)
                    crit_tensor_counter += 1

                crit_tensor_counter=0
                for tensor in critic_model.trainable_variables:
                    tf.summary.histogram('critic tensor {}'.format(crit_tensor_counter), tensor, step=epoch)
                    crit_tensor_counter += 1
                ...
                try:
                    ...
                    for tensor in gen_gradients:
                        tf.summary.histogram('generator gradients tensor {}'.format(gen_tensor_counter), tensor, step=epoch)
