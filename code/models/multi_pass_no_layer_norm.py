...
Thereby areas which are sufficiently sampled in the low-res simulation are sufficiently sampled.
Here we only use residual learning in a given block without a shortcut through the whole network.
...
def first_critic(input_batch, lr_batch, factor=2):
    ...
    X = layers.Conv2D((8), (3, 3), padding = 'same', kernel_initializer = keras.initializers.RandomNormal(), name = 'factor2crit5')(X)
    X = layers.LeakyReLU()(X)

    X = layers.Conv2D((2), (3, 3), padding = 'same', kernel_initializer = keras.initializers.RandomNormal(), name = 'factor2crit6')(X)
    X = layers.LeakyReLU()(X)    
...
def second_generator(input_batch, factor=2):
    ...
    X = second_res_block_mult(X, 3, 16, 8)
...
def second_critic(input_batch, lr_batch, factor=2):
    X = layers.Conv2D((8), (1, 1), padding = 'same', kernel_initializer = keras.initializers.RandomNormal(), name = 'factor2crit1')(input_batch)
    X = layers.LeakyReLU()(X)
    ...
    X = layers.Conv2D((16), (5, 5), padding = 'same', kernel_initializer = keras.initializers.RandomNormal(), name = 'factor2crit6')(X)
    X = layers.LeakyReLU()(X)
