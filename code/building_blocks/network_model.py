...
class network_model:
    ...
    def __init__(self, network, optimizer, in_x=None, in_y=None, in_z=None, channels=1, dim3=False):
        if dim3 == False:
            self.input = layers.Input(shape=(in_x,in_y,channels))
        else:
            self.input = layers.Input(shape=(in_x,in_y,in_z,channels))
        
        self.model = tf.keras.Model(inputs = self.input, outputs = network(self.input))
