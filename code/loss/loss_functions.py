...
def first_generator_loss(hr_tiles, sr_tiles, lr_tiles, first_critic_model):
    ...
    l1_penalty = tf.norm(difference, axis=[1,2])
    return -tf.reduce_mean(first_critic_model((sr_tiles, lr_tiles)))+Lambda*tf.reduce_mean(l1_penalty)
...
def second_generator_loss(hr_tiles, sr_tiles, lr_tiles, second_critic_model):
    ...
    difference = sr_tiles-hr_tiles
    l1_penalty = tf.norm(difference, axis=[1,2])
    return -tf.reduce_mean(second_critic_model((sr_tiles, lr_tiles)))+Lambda*tf.reduce_mean(l1_penalty)