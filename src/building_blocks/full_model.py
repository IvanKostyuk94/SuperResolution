...
class full_model:
    ...
    def __init__(self, first_generator, loss, first_critic=None, second_generator=None, second_critic=None, is_multipass=False, is_gan=False):
    ...
    def _number_of_nets(self):
        if self.second_generator==None:
            no_gen = 1
        else:
            no_gen = 2
        if self.first_critic==None:
            no_crit = 0
        else:
            if self.second_critic==None:
                no_crit = 1
            else:
                no_crit = 2
    ...
    def generator_step(self, lr_tensor, hr_tensor, first_or_second='first'):
        if self.is_gan==True:
            raise NotImplementedError('This is a gan model a generator step is only implemented in combination with a critic step')
        else:
            if first_or_second == 'first':
                ...
                raise NotImplementedError('The generator network has to be the first of the second network')

            with tf.GradientTape() as gen_tape:
                sr_tensor = generator.apply(lr_tensor)
                loss = self.loss.apply(sr_tensor, hr_tensor, lr_tensor, loss_type)
    ...
    def critic_step(self, lr_tensor, hr_tensor, first_or_second='first'):
        if self.is_gan==False:
            raise NotImplementedError('This is not a gan model therefore a critic step is not implemented')
        else:
            if first_or_second == 'first':
                generator = self.first_generator
                ...
                raise NotImplementedError('The critic network has to be the first of the second network')
    ...
    def full_step(self, lr_tensor, hr_tensor, first_or_second='first'):
        if self.is_gan==False:
            raise NotImplementedError('This is not a gan model use "generator_step" to optimize this model')
        else:
            if first_or_second == 'first':
                generator = self.first_generator
                critic=self.first_critic
                ...
                raise NotImplementedError('The first_or_second variable has to be set to either "first" or "second"')
            
            with tf.GradientTape() as gen_tape, tf.GradientTape() as crit_tape:
                sr_tensor = generator.appy(lr_tensor)
                gen_loss = self.loss.apply(sr_tensor, hr_tensor, lr_tensor, gen_loss_type)
                crit_loss = self.loss.apply(sr_tensor, hr_tensor, lr_tensor, crit_loss_type)
        ...
        critic.optimizer.apply_gradients(zip(crit_gradients, critic.trainable_variables))
