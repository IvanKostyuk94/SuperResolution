...
# Class which tracks the whole run i.e. it creates the folder to write the checkpoints to and writes loss, gradients etc. to the corresponding files 
class run_tracker:
    
    def __init__(self, run_dir, track_loss=True, track_gradient=True, tracked_loss_prop=['epoch', 'avg_loss_gen1'], tracked_grad_prop=['epoch', 'avg_grad_gen1']):
        ...
        if self.track_loss:
            ...
            if not os.path.isfile(self.loss_file):
                with open(self.loss_file, 'w', newline='') as loss_file:
                    ...
                    writer.writerow(self.tracked_loss_prop)(f'Created the rundirectory at {self.run_dir}')
    ...
    def update_loss(self, data):
        if len(data) == len(self.tracked_loss_prop):
            ...
            raise ValueError(f'Length of data ({len(data)}) must be equal to the number of properties being tracked ({len(self.tracked_loss_prop)})')
