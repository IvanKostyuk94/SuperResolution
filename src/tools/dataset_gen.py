...
def create_dataset(
    simulation, random_array, output_dir, output_name, grid_size=64
):
    k = 0
    for i in range(8):
        for j in range(8):
            for l in range(8):
                cube = simulation[
                    ...
                    l * grid_size : (l + 1) * grid_size,
                ]

                np.save(output_dir + output_name + str(random_array[k]), cube)
                k += 1
                np.save(
                    output_dir + output_name + str(random_array[k]),
                    np.flip(cube, 0),
                )
                k += 1
                np.save(
...
create_dataset(
    lr_sim_in,
    numbering,
    "/u/ivkos/sr/TrainingTiles/lr_no_scale/",
    lr_output_name,
)
create_dataset(
    hr_sim_in,
    numbering,
    "/u/ivkos/sr/TrainingTiles/hr_no_scale/",
    hr_output_name,
)
exit()

lr_sim, hr_sim, output_lr, output_hr = normalize_dataset(
    lr_sim_in, hr_sim_in, scale_2, 2
)
create_dataset(lr_sim, numbering, output_lr, lr_output_name)
create_dataset(hr_sim, numbering, output_hr, hr_output_name)

lr_sim, hr_sim, output_lr, output_hr = normalize_dataset(
    lr_sim_in, hr_sim_in, scale_3, 3
)
create_dataset(lr_sim, numbering, output_lr, lr_output_name)
create_dataset(hr_sim, numbering, output_hr, hr_output_name)

lr_sim, hr_sim, output_lr, output_hr = normalize_dataset(
    lr_sim_in, hr_sim_in, scale_4, 4
)
create_dataset(lr_sim, numbering, output_lr, lr_output_name)
create_dataset(hr_sim, numbering, output_hr, hr_output_name)
